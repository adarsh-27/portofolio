...
export function Globe({ globeConfig, data }) {
  ...
  const _buildData = () => {
    ...
    for (let i = 0; i < arcs.length; i++) {
      ...
      if (
        ...
        console.warn("Invalid data detected:", arc);
    ...
    // Remove duplicates for the same lat/lng
  ...
  useEffect(() => {
    if (globeRef.current && globeData) {
      ...
      globeRef.current.arcsData(validData);
    }
  }, [globeData]);
  ...
  const startAnimation = () => {
    ...
    globeRef.current
      ...
      .arcColor((e) => e.color)
      .arcAltitude((e) => {
        return isNaN(e.arcAlt) ? 0.1 : e.arcAlt * 1; // Default to 0.1 if NaN
      })
      .arcStroke((e) => {
        return [0.32, 0.28, 0.3].includes(e.arcStroke) ? e.arcStroke : 0.3; // Default to 0.3
      })
